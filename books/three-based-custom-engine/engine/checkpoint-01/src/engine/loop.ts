...
import { createClock, resetClock, tickClock } from './clock.ts';
...
export interface Loop {
  ...
  resetTime(): void;
  /**
   * Вернуть цикл в состояние до первого кадра: номер кадра, счётчик шагов,
   * часы и аккумулятор. Подача кадров при этом прекращается.
   */
  reset(): void;
}
...
export function createLoop(options: LoopOptions): Loop {
  ...
  return {
    ...
    reset() {
      // Сбрасывается всё, от чего зависит кадр, а не только время: иначе
      // второй прогон продолжит первый (журнал 2026-07-31, глава 3).
      if (running) {
        running = false;
        source.cancel(handle);
      }
