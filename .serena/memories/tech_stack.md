...
- **Контур M (Mac M3) — только для asm-книги** (решение владельца 2026-07-30: Rust-заходы ведутся на винде, rustup на мак не ставится): macOS 15.7.4, Apple clang 17, сборка x86-64 через `-arch x86_64` + Rosetta 2, `as`, `objdump` (LLVM-версия), `lldb` из коробки; `gdb` отсутствует. Данные — по §7.1 `books/asm-book/asm-book-requirements.md`; в журнал §10 C-книги ещё не перенесены (открытый вопрос В2).
